...
obj_name_div = $(
  '<div id="permdialog_objname" class="section"><b>File Path:</b><span id="permdialog_objname_namespan"></span> </div>'
);
...
file_permission_users = define_single_select_list(
  ...
  function (selected_user, e, ui) {
    if (selected_user && selected_user.length > 0) {
      // When a new user is selected, change username attribute of grouped permissions and show it
      grouped_permissions.attr("username", selected_user);
      grouped_permissions.show();
    } else {
      // Hide grouped permissions if no user is selected and reset the username attribute
      grouped_permissions.hide();
...
perm_remove_user_button = $(
  '<button id="perm_remove_user" class="ui-button ui-widget ui-corner-all">Delete User/Group</button>'
);
